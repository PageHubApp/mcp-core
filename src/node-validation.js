...
const HARDCODED_COLOR_RE = /(?:^|\s)(?:bg|text|border|ring|shadow|outline|divide|from|to|via)-\[#[0-9a-fA-F]{3,8}\]/g;
const SLATE_COLOR_RE = /(?:^|\s)(?:bg|text|border|ring)-(?:slate|gray|zinc|neutral|stone|red|orange|amber|yellow|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)-\d{2,3}(?:\/\d+)?/g;

const DAISY_TOKEN_MAP = {
  'bg-': 'bg-base-100 / bg-base-200 / bg-primary / bg-secondary / bg-accent / bg-neutral',
  'text-': 'text-base-content / text-primary-content / text-secondary-content / text-neutral-content',
...
const VALID_TAG_NAMES = new Set(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'span', 'div', 'Textfit']);

// ── Structural nodes that should never be auto-fixed ──

const STRUCTURAL_NODE_IDS = new Set([
  'ROOT', 'page_home', 'hdr_root', 'hdr_section', 'hdr_inner',
  'ftr_root', 'ftr_content', 'ftr_inner',
]);
...
function validateNodes(flatMap, opts = {}) {
  ...
  for (const nodeId of nodeIds) {
    ...
    if (resolvedName === 'Image') {
      if (props.src && !props.content) {
        if (autoFix) {
          ...
          if (!props.type && typeof props.content === 'string' && props.content.startsWith('http')) {
            props.type = 'url';
          }
          fixes.push(`🔧 ${nodeId}: Migrated Image prop "src" → "content" (type: "${props.type || 'cdn'}")`);
        } else {
          errors.push(`❌ ${nodeId}: Image uses "src" prop — must use "content" (+ type: "url" for external URLs)`);
        }
      }
      if (!props.content && !props.src) {
        warnings.push(`⚠️ ${nodeId}: Image has no content/src — will render empty`);
      }
      if (props.content && typeof props.content === 'string' && props.content.startsWith('http') && !props.type) {
        if (autoFix) {
          props.type = 'url';
    ...
    if (resolvedName === 'Text') {
      if (!props.tagName) {
        if (autoFix) {
          ...
          if (/text-(?:5xl|6xl|7xl|8xl|9xl|\[.*\])/.test(cn) || /font-black|font-extrabold/.test(cn)) {
            props.tagName = 'h1';
          } else if (/text-(?:3xl|4xl)/.test(cn)) {
            props.tagName = 'h2';
          } else if (/text-(?:xl|2xl)/.test(cn) && /font-bold|font-semibold/.test(cn)) {
            props.tagName = 'h3';
          } else if (/text-\[(?:9|10|11|12)px\]|text-xs|tracking-\[/.test(cn) || /uppercase/.test(cn)) {
            props.tagName = 'span';
          } else {
            props.tagName = 'p';
          }
          fixes.push(`🔧 ${nodeId}: Set missing tagName to "${props.tagName}" (inferred from className)`);
        } else {
          errors.push(`❌ ${nodeId}: Text missing required "tagName" prop (h1-h6, p, span, div)`);
        }
      } else if (!VALID_TAG_NAMES.has(props.tagName)) {
        warnings.push(`⚠️ ${nodeId}: Text has invalid tagName "${props.tagName}" — valid: ${[...VALID_TAG_NAMES].join(', ')}`);
      }
    ...
    if (!isStructural && resolvedName !== 'Background' && resolvedName !== 'Header' && resolvedName !== 'Footer') {
  ...
  if (warnings.length > 0) {
    ...
    if (colorWarnings.length > 0) {
      ...
      if (colorWarnings.length > 5) {
        lines.push(`  ...and ${colorWarnings.length - 5} more. Run with DaisyUI tokens for proper theme support.`);
      }
    }
  }
