...
function buildButtonClassFramework(args = {}) {
  ...
  const emphasis = String(args.emphasis || "default")
    .trim()
    .toLowerCase();
...
function validateButtonClasses(args = {}) {
  ...
  if (inferredVariant === "outline" || variantHint === "outline") {
    ...
    if (!hasToken(tokens, "border-base-content/30")) {
      addIssue(
        ...
        "warn",
        "Outline button should use `border-base-content/30`."
      );
      if (autoFix) {
    ...
    if (nextModifiers.includes("btn-neon")) {
      addIssue(
        "outline-neon-modifier",
        "warn",
        "Outline button should not keep `btn-neon` active."
      );
      if (autoFix) {
  ...
  if (inferredVariant === "primary" || variantHint === "primary") {
    ...
    if (hasToken(tokens, "btn-outline")) {
      addIssue(
        "primary-outline-conflict",
        "warn",
        "Primary button should not include `btn-outline`."
      );
      if (autoFix) {
        tokens = tokens.filter(t => t !== "btn-outline");
