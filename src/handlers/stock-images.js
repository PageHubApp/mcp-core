...
module.exports = {
  ...
  async find_image(args) {
    ...
    if (!q && !category) {
      throw new Error('Provide q (search keywords) and/or category (hero, avatar, product, background, team, general).');
    }
...
function formatResults(images) {
  const lines = images.map((img) => {
    const dims = img.width && img.height ? ` (${img.orientation || 'landscape'}, ${img.width}x${img.height})` : '';
